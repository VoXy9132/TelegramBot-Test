...
def readAnswer(Number):
    file = open(r'Files\Text\Quests1-12.txt', mode='r', encoding='utf-8')
    Read = False
    Text = []
    for text in file:
        if not Read:
...
def white_root(filetxt, kword, root='False'):
    file = open(f'Files\Text\{filetxt}', mode='r', encoding='utf-8')
    Dict = dict()
    for num in file:
        Dict[num.strip().split(':')[0]] = num.strip().split(':')[1]
    file.close()
    file = open(f'Files\Text\{filetxt}', mode='w', encoding='utf-8')
    Dict[kword] = root
    for num in Dict:
...
def callback(call):
    if not read_root('Files/Text/Root', 'quest_by_number'):
        help_list = readlist('Files/Text/Help', 'Help')
        quest_list = readlist('Files/Text/List_Quest', 'Quest')
        if call.data == 'name_help':
            ...
            bot.send_message(call.message.chat.id, 'Эту команду надо вводить в чат. Сама кнопка не имеет важной функции')
...
def messages(message):
    ...
    print(read_root('Files\Text\Root', 'quest_by_number'))
